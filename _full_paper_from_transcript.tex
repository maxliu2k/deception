...
For each source LLM, we trains an independent action network and price network. The action classifier is a $32\!\to\!32\!\to\!32\!\to\!3$ MLP with sigmoid hidden activations and dropout $0.2$. It predicts continue, accept, or reject using weighted cross-entropy. For class $c$ with $n_c>0$ training examples, the fold-specific weight is
\begin{equation}
w_c=\min\!\left(3,\sqrt{n_{\max}/n_c}\right),
\end{equation}
which raises the influence of terminal actions without fully inverse-weighting the 31 sparse rejects. The price regressor has the same widths with ReLU hidden activations and a softplus output. It is trained only on continue actions, using Huber loss ($\delta=0.1$), to predict proposed price divided by the acting agent's private bound. Inputs are standardized using training-fold statistics. Both networks use AdamW (learning rate $10^{-3}$, weight decay $10^{-4}$), batch size 32, at most 300 epochs, and early stopping with patience 40.
...
%suggesting successful capture of policy-level negotiation behavior.


%\subsubsection{Summary of Results}

%Across models, we observe a tradeoff between agreement rate and surplus maximization. Efficient policies increase closure probability but reduce extracted surplus, while analytical policies converge toward stable equilibrium pricing. Cooperative policies balance both objectives but exhibit higher variance.
...
Table~\ref{tab:neg_behavior} characterizes each source model in both roles on the five held-out seeds (25 episodes per model/role). The buyer and seller columns are not symmetric duplicates: they measure the source model against all five opponents while occupying the indicated role. Grok and Opus are the strongest buyers, achieving nearly identical agreement ($92\%$) and normalized buyer utility ($0.661$ and $0.658$); their accepted prices allocate only $z=0.282$ and $0.285$ of the available surplus to the seller. In the seller role, GPT, Grok, and Opus form a middle cluster near $u_S=0.43$--$0.44$. Pro closes most often as seller ($96\%$) but captures only $u_S=0.194$ and $z=0.202$, a high-closure, buyer-favorable signature. Llama is the clearest outlier: as seller it agrees in only $72\%$ of episodes and explicitly rejects $28\%$, compared with $4$--$12\%$ rejection for the other seller profiles.
...
Opus    & 92 & 0.658 & 88 & 0.438 \\
Pro     & 84 & 0.437 & 96 & 0.194 \\
\hline
\end{tabular}
\end{table}
...
Table~\ref{tab:neg_baseline} screens each mimic against the deterministic mathematical policy in both roles (15 episodes per cell: five seeds and three mimic replicates). Buyer performance separates sharply: Opus and Grok retain $u_B=0.742$ and $0.737$, with seed-clustered intervals $[0.595,0.870]$ and $[0.610,0.887]$, while Pro reaches only $0.288$. Grok is also the strongest seller ($u_S=0.609$); Llama and Pro concede most of the surplus as sellers ($0.253$ and $0.170$). Mathematical self-play always agrees, with $z=0.412$, allocating normalized utilities $u_B=0.588$ and $u_S=0.412$. These are mimic-mediated screening results. Unlike the auction and deception ladders, they have not yet been repeated against the real LLMs, so we do not claim that the ranking transfers.
...
Opus    & 100.0 & 0.742 & 93.3 & 0.435 \\
Pro     & 86.7 & 0.288 & 86.7 & 0.170 \\
\hline
\end{tabular}
\end{table}
...
\hline
Pro   & 29.6 [25.5, 33.6] & 0.86 & 0.56 & 0.56 \\
Opus  & 24.7 [20.5, 28.9] & 0.86 & 0.60 & 0.59 \\
...
Pro and Opus inflate the low-truth attributes the hardest (over-promising by $0.56$ and $0.60$ on attributes with $t\le0.3$), drive their credibility down to $\approx0.56$--$0.59$, and win the most rounds ($29.6\%$ and $24.7\%$) on the strength of the raw claim height. Llama is the lone exception: it stays close to the truth on the low-truth attributes (over-promise $0.09$), keeps by far the highest credibility ($0.82$), and lands at the fair share ($20.3\%$), credible but never claiming high enough to dominate. GPT and Grok inflate the low-truth attributes moderately and end with credibility as damaged as Pro and Opus but without the winning claim height, finishing last ($13.1\%$ and $12.3\%$). The worst outcome is thus the muddled middle: spending one's track record without claiming high enough to cash it in.
...
Did anything change? I don't see anything visibly different from two turns ago.